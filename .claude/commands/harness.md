...
> CLI 레퍼런스·환경변수·테스트 방법은 [README.md](../../README.md) 를 참고하라. 이 파일은 step 설계 규칙과 워크플로에 집중한다.
...
각 step 실행이 끝나면 execute.py 가 `phases/{task-name}/step{N}-output.json` 을 자동 생성한다. 여기에는 `git_sha_before`, `git_sha_after`, Claude 세션이 반환한 JSON(`claude` 필드) 이 담겨 재현성 추적에 쓰인다. 이 파일은 사람이 손대지 않는다.
...
파일 작성 후 실행 전에 아래 명령으로 스키마와 가드레일 placeholder 를 점검한다. 통과해야 실행 단계로 넘어간다.

```bash
python scripts/execute.py validate {task-name}
```
...
| 명령 | 용도 |
|---|---|
| `python scripts/execute.py {task-name}` | 순차 실행 (로컬 커밋만) |
| `python scripts/execute.py {task-name} --push` | 실행 후 origin 으로 push |
| `python scripts/execute.py status` | 모든 phase 진행 상황 조회 |
| `python scripts/execute.py validate {task-name}` | 실행 없이 스키마/가드레일 검증만 |
| `python scripts/execute.py unblock {task-name}` | blocked step 들을 pending 으로 되돌림 (재개용) |
...
- **blocked 발생 시**: `blocked_reason`에 적힌 사유를 해결한 뒤, `status`를 `"pending"`으로 바꾸고 `blocked_reason`을 삭제한 뒤 재실행한다. blocked step 이 여러 개라면 `python scripts/execute.py unblock {task-name}` 으로 한 번에 되돌릴 수 있다.
