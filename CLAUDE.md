...
- 학습 카드(`assets/study/NN_<slug>.json`, NN=01~16) 는 사람이 직접 작성한다. 파일명 슬러그와 `lib/services/study_card_service.dart` 의 `topics` 리스트가 1:1 로 일치해야 한다. 학습 카드는 한국어 단일 언어로 작성되며 (다국어는 추후 작업), 카드 스키마 변경 시 `lib/models/study_card.dart` 와 `lib/screens/study_card_screen.dart` 를 함께 갱신한다.
