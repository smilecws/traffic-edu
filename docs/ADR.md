...
**이유**: 문제 은행과 동일하게 오프라인·번들 접근 방식을 유지. 별도 CMS 없이 파일 편집만으로 콘텐츠 추가 가능. 학습 토픽 16개는 출제 영역 중심으로 분류. 초기에는 LocalizedText 다국어를 시도했으나 콘텐츠 작성·검수 부담이 커서 한국어 단일로 단순화.
...
### ADR-016: Web 클라이언트 App Check 도입 (reCAPTCHA v3)
**결정**: Web 빌드에 한해 `firebase_app_check` 를 추가하고 `ReCaptchaV3Provider` 로 activate 한다. 사이트 키는 `--dart-define=RECAPTCHA_V3_SITE_KEY=...` 빌드 인자로 주입하며 `main.dart` 의 `_recaptchaV3SiteKey` 상수가 받는다. 키가 비어 있으면 activate 를 silent skip 한다 (로컬 dev / 키 미등록 빌드). Android/iOS 의 Play Integrity / DeviceCheck 는 이번 phase 범위 외 — P2-2 (앱스토어 배포 준비) 와 함께 처리한다. 보조 방어선으로 `firestore.rules` 의 `user_answers` create 에 페이로드 검증(필드 집합 정확 일치·타입·`score`/`total` 범위·`items.size() == total`) 을 추가했다.
**이유**: 클라이언트 read 는 외부 집계로 0 이라(ADR-013) read 남용 위험은 이미 해소돼 있지만, 봇이 REST API 로 익명 로그인 + `user_answers` 에 가짜 세션을 적재해 Spark write 한도를 소진하고 통계를 오염시키는 시나리오는 남아 있었다. App Check 는 봇이 attestation 토큰을 받지 못하게 해 write 자체를 차단한다.
**트레이드오프**: reCAPTCHA v3 는 사용자에게 보이지 않지만 Google 도메인으로 트래픽이 가므로 사적/특수 네트워크에서 점수가 낮게 나올 가능성이 있다. 따라서 Firebase Console 의 enforcement 는 **모니터링** 단계로 시작해 1~2주 정상 트래픽을 확인한 뒤 **enforce** 로 전환한다. `firestore.rules` 에는 `request.app != null` 을 박지 않았다 — 룰에 박으면 콘솔 enforcement 토글이 무력화돼 모든 write 가 한 번에 실패할 위험이 있고, 단계적 적용도 불가능해진다. 페이로드 검증은 App Check 가 우회되더라도 형식이 맞지 않는 가짜 세션을 룰 단에서 거르는 한 줄 방어선이다.
