...
- `question_stats` 컬렉션은 P0-2 옵션 C 적용으로 폐기됐다. 더 이상 클라이언트가 쓰지 않으므로 삭제 대상에서 제외한다.
...
3. Firebase 콘솔 확인:
   ...
   - Firestore — `question_stats` 컬렉션이 생성되지 않았는지 (P0-2 적용 후 클라이언트가 쓰지 않아야 정상)
4. F12 Console 에 빨간 에러 없는지

## 6. 첫 진짜 사용자 받기

알림 보내기 전 마지막 확인:
- [ ] `question_stats` 컬렉션 미존재 (P0-2 옵션 C 적용 후 폐기됨)
- [ ] user_answers 비어있음
...
> **2026-05 완료** — PR #7 (`42819fe`). 옵션 C(write 통합) 적용.
>
> - 클라이언트는 `user_answers/{uid}/sessions/{auto_id}` 에 세션당 **1 write** 만 한다 (`5b6a002`).
> - `aggregate_stats.js` 가 `user_answers` 세션 로그를 집계 소스로 사용 (`74a5277`, `34941e7`).
> - `firestore.rules` 의 `question_stats` 블록 제거 (`2f862e7`). 2026-05-26 Firebase 콘솔에서 게시 완료, 기존 `question_stats` 컬렉션 삭제 완료.
> - Spark 일일 한도(20,000 write) 감당 가능 사용자: **약 488명 → 약 20,000 세션/일** 로 확대.

이하 의사결정 기록(역사적 참고용):

**옵션 C: write 통합 — `question_stats` 직접 write 제거** (채택)

P0-4 에서 read 를 외부 집계로 풀었듯, write 도 같은 발상으로 풀었다.
...
**완료된 조치**
- [x] `lib/services/global_answer_stats_service.dart` 의 `applySessionResults` 에서 `question_stats` batch write 제거 (`5b6a002`)
- [x] `tool/aggregate_stats.js` 의 집계 소스를 `question_stats` → `user_answers` 의 전체 `sessions` 로 전환 (`74a5277`)
- [x] `firestore.rules` 에서 `question_stats` 의 클라이언트 write 권한 제거 (`2f862e7`)
- [x] 배포된 Firestore 룰 갱신 (2026-05-26 콘솔에서 게시) + 기존 `question_stats` 컬렉션 삭제
- [ ] 주의: `aggregate_stats.js` 가 `user_answers` 의 모든 `sessions` 를 read → GitHub Actions read 비용 증가. 사용자가 많아지면 cron 간격을 4시간 → 8시간으로 조정 (모니터링 필요)

**옵션 A: Blaze 전환** (미채택 — 옵션 C 가 더 안전)
- write 비용 자체는 미미: 5,000명/월 × 41 write ≈ 205,000 write/월 → 약 $0.37/월. 막히는 건 비용이 아니라 Spark 의 일일 한도였다.
- 옵션 C 로 한도 자체를 회피했으므로 Blaze 전환은 당분간 불필요.

**옵션 B: Spark 유지 + 샘플링** (미채택)
- `applySessionResults` 에 샘플링 게이트를 둬 `question_stats` write 를 일부만 수행. 통계 정확도가 떨어진다. 옵션 C 가 정확도 손실 없이 더 우수했다.
...
### ✅ P0-4. Firestore read 한도 대응 — 완료 (클라이언트 Firestore read 0)

> **2026-05 완료** — Step 1·2·3 모두 적용. 클라이언트의 Firestore read 는 **0** 으로 축소됐다.
>
> - Step 1 (영구 캐시): `lib/services/global_answer_stats_service.dart` 에 SharedPreferences 1시간 TTL 캐시.
> - Step 2 (외부 집계): `.github/workflows/aggregate_stats.yml` + `tool/aggregate_stats.js` 가 `user_answers` 를 집계해 `data-aggregates` 브랜치에 `aggregates.json` 커밋. 클라이언트는 raw URL HTTP fetch. 마지막 집계 2026-05-20.
> - Step 3 (외부 리포트): 같은 워크플로가 `full_report.json` + `report.md` 도 함께 생성.
...
#### Step 1. SharedPreferences 영구 캐시 (단기 효과) — ✅ 완료
- [x] `lib/services/global_answer_stats_service.dart`에 SharedPreferences TTL 캐시 추가 (1시간)
- [x] 우선 캐시 표시 → 백그라운드 fetch → 성공 시 갱신 패턴
- [x] UI에 "최근 업데이트: N시간 전" 표시
- 효과: 사용자당 일일 1~2회 read만 발생, **빈도 80~90% 절감** (Step 2 적용으로 클라이언트 Firestore read 자체가 0 이 됨)

#### Step 2. 외부 집계 도입 — GitHub Actions cron + 별도 브랜치 (근본 해결) — ✅ 완료
...
**구체적 조치**
- [x] Firebase Console → 프로젝트 설정 → 서비스 계정 → 새 비공개 키 생성 (JSON)
- [x] GitHub 저장소 Settings → Secrets → `FIREBASE_SERVICE_ACCOUNT` 추가
- [x] `.github/workflows/aggregate_stats.yml` 작성 (단, **테스트 단계라 cron 은 주석 처리**, 배포 직전 4-1 절차로 활성화 필요)
- [x] `tool/aggregate_stats.js` 집계 스크립트 작성 (집계 소스는 P0-2 옵션 C 적용으로 `user_answers` 사용)
- [x] `data-aggregates` 브랜치 초기화
- [x] Flutter 클라이언트 변경:
  - [x] `pubspec.yaml`에 `http` 패키지 추가
  - [x] `lib/services/global_answer_stats_service.dart` 가 raw URL HTTP fetch + SharedPreferences 1시간 TTL 캐시 + 만료 SP 폴백 + empty 폴백 4단계 사용
  - [x] aggregates.json 스키마: `{ "updated_at", "hardest_top10", "all_questions" }` (P0-2 에서 `all_questions` 추가)
  - [x] StatsScreen 동작 검증 완료
...
**구체적 조치**
- [x] `tool/aggregate_stats.js`에 사용자 수 집계 추가 (`fetchUserCounts()` — `count()` aggregation 사용)
- [x] TOP 20 추출 로직 (`buildHardestList(stats, 20)`)
- [x] Markdown 템플릿 렌더링 (`buildReportMd()`)
- [x] 3개 파일 모두 `data-aggregates` 브랜치에 commit (워크플로 last 5단계)
- [ ] `report.md` 공유 링크 검증 (배포 후): `https://github.com/smilecws/quiz/blob/data-aggregates/report.md`
- [ ] (선택) 문제 ID 클릭 시 question_detail 화면으로 연결되는 deep link 추가 검토
...
| 4 | P0-2 Blaze 전환 결정 | 30분~2시간 | P0-4 | 옵션 C 채택으로 불필요 |
| 5 | P0-3 라이선스 정비 (LICENSE/README/출처표기) | 1~2시간 | - | 일부 완료 |
